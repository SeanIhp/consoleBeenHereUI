...
export default {
        ...
        buttons: [[
        {
            ...
            action: function(){
                ...
                if(_that.body.config.list.rowDatas.length==0 || 
                    ...
                    let r = merge([{}, _that.body.config.newone], {depth: 12, types: { object: true, array: true }});
                    r._flag = 'B';
                    ...
                    if(!!_that.body.config.list.theRow[_that.body.config.list.key]){
                        for(let i=0; i<_that.body.config.list.rowDatas.length; i++){
                            if(_that.body.config.list.rowDatas[i][_that.body.config.list.key]==_that.body.config.list.theRow[_that.body.config.list.key]){
                                ...
                                let obj = merge([{}, _that.body.config.list.rowDatas[i]], {depth: 12, types: { object: true, array: true }})
                                _that.body.config.list.deletedRowDatas.push(obj);                                
  ...
  methods: {
    ...
    oRowDblClick: function(currentRow, index){
        this.body.config.list.theRow = currentRow;
        // this.body.config.list.theRow._ready2Edit = true; //使Cellet生效  
        console.log("oRowDblClick: ", currentRow, this);    
        this.view.dialog.form.dataSourceType = 'detail';
        this.view.dialog.form.dataSourceKey = this.view.list.detail.bodyActivity;
        let det = null;
        ...
        if(det!=null){
            det.config.list.currentRow = merge([{}, currentRow], {depth: 12, types: { object: true, array: true }});
            this.view.dialog.form.sourceData = det.config.list.currentRow;
            this.view.dialog.form.fields = det.config.list.columns;
            this.view.dialog.form.rules = det.config.list.rules;
        }
        this.view.dialog.form.visible = true;
    }
  },  
  watch: {
