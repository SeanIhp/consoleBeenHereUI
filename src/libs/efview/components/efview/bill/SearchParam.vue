<template>
    <!-- 搜索条件 -->
    <Form :label-width="100" :model="searchParam" v-if="searchParam && searchParam.rows">
        <Row :gutter="16" style="margin-bottom:10px;" v-for="(data,index) in searchParam.rows">
            <Col :span="5" v-for="item in searchData">
                    <FormItem :label="item.title" v-if="item.belongs == 'parent'">
                        <Input type="text" v-model="data[item.key]" v-if="item.type===undefined || item.type==='text' || item.type==='email'"></Input>
                        <Select v-else-if="item.type==='select'" :label-in-value="true" placeholder="请选择" v-model="data[item.key]" @on-change="selectChange(data,item.key,index)">
                            <Option v-for="opt in view.dict[item.extra].data" :value="opt.code" :key="opt.code">
                                {{ opt.name }}
                            </Option>
                        </Select>
                    </FormItem>
                    <FormItem :label="item.title" v-else-if="item.belongs == 'child'">
                        <Input type="text" v-model="data[item.key]" v-if="item.type===undefined || item.type==='text' || item.type==='email' || data.selectItem.showInput" @on-change="inputChange(data)"></Input>
                        <Select v-else-if="item.type==='select' || data.selectItem.showSelect " :label-in-value="true" placeholder="请选择" v-model="data[item.key]" @on-change="selectValueChange(data)">
                            <Option v-for="opt in data.selectItem.types" :value="opt.value" :key="opt.value">
                                {{ opt.label }}
                            </Option>
                        </Select>
                    </FormItem>
                    <FormItem :label="item.title" v-else>
                        <Input type="text" v-model="data[item.key]" v-if="item.type===undefined || item.type==='text' || item.type==='email'"></Input>
                        <Select v-else-if="item.type==='select'" :label-in-value="true" placeholder="请选择" v-model="data[item.key]">
                            <Option v-for="opt in view.dict[item.extra].data" :value="opt.code" :key="opt.code">
                                {{ opt.name }}
                            </Option>
                        </Select>
  ...
  methods: {
    selectChange: function(Option,key,index) {
        ...
        if(item) {
            ...
            Option.value2 = '';
            Option.value3 = '';
            _that.view.list.searchData.forEach(element => {
                if(element.belongs == 'child' && item.showSelect) {
                    element.type = 'select';
                    //_that.view.list.value.type = 'select';
                    setTimeout(function() {
