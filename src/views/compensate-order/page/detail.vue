    ...
    <div class="bgc-F0F0F0">
        <Row>
            ...
            <Card style="overflow: hidden;">
                <Button v-if="$route.query.isAudit" class="handle-btn" type="primary" @click="modifyCompensateOrder('infoFrom')">修改</Button>
                <Button v-if="$route.query.isAudit" class="handle-btn" type="primary" @click="saveCompensateOrder('infoFrom')">保存</Button>
                <Button v-if="$route.query.isAudit" class="handle-btn" type="primary" @click="saveCompensateOrderlinkToList('.infoFrom')">保存&关闭</Button>
                <Button v-if="$route.query.isAudit" class="handle-btn" type="primary" @click="audit('infoFrom')">审核</Button>    
                <Button class="handle-btn-right" :class="{'handle-btn-margin': !$route.params.isAudit}" type="ghost" @click="linkToList">返回</Button>
            </Card>

            <Card class="margin-top-10">
              <p slot="title">单据信息</p>
              <Form v-if="$route.query.isAudit" ref="infoFrom" :model="infoParams" :rules="ruleValidate" :label-width="100" > 
              <Row :gutter="10">
                <Col span="8">
                ...
                <Col span="8">
                    <FormItem label="退货申请单号"  prop="applysheetno">
                        <Input type="text" v-model="infoParams.applysheetno" @input='searchApplysheetno'  placeholder="" style="width: 100%;"></Input>
                    </FormItem>
                </Col>
                <Col span="8">
...
export default {
	...
	data() {
        ...
        let isValidateSubClass = (rule, value, callback) => {
            if ((this.infoParams.compensate_type.indexOf('券') > -1 || this.infoParams.compensate_type.indexOf('积分') > -1) && this.infoParams.compensate_subclass === '') {
                console.log(rule)
                //this.compensate_subclass[0].required = false;
    ...
    mounted() {
        ...
        this.infoParams.status = this.$route.query.isAudit?0: 1;
        console.log(this.infoParams.status)
        this.getDetailInfo()
    },
	computed: {},
	methods: {
        ...
        // 修改补偿单
        modifyCompensateOrder(name) {
            this.submitInfoRefresh("https://rap-api.sjhgo.com/app/mock/data/190", 1)
        },

		// 保存补偿单
		saveCompensateOrder(name) {
    ...
    watch: {
        ...
        //     if (from.params.isAudit !== '') {
        //         this.infoParams.status = from.params.isAudit?"0": "1";
        //     }
        //     // if (from.is)
        // }    
    }
