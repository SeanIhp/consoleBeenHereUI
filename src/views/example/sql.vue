...
export default {
  ...
  methods: {
    ...
    btnCreatSQL() {
      ...
      this.$http({
        url: "/ampOpenapiService?method=user.app.search&token=1596625913905280171",
        method: "post",
        data: param
      }).then(res => {
        console.log("returnData:", res); //res返回结果
        this.sqlOutPut = ""; //sql生成结果赋予值
      });

      //   const that = this;
      //   request(
      //     {
      //       url: "/ampOpenapiService?method=user.app.search&token=", //API请求地址
      //       method: "post",
      //       data: param
      //     },
      //     function(data) {
      //       console.log(data); //打印API返回结果
      //       that.sqlOutPut = ""; //sql生成结果赋予值
      //     }
      //   );
      //－－－－－－－－－－API异步请求end－－－－－－－－－－
    },
